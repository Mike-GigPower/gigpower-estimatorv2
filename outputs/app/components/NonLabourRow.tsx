...
export default function NonLabourRow({
  ...
  return (
    <tr data-blank={blank ? "true" : "false"}>
      <td>
        <span className="print-only">{line.title?.trim() || "—"}</span>
        <input
          className="no-print"
          value={line.title}
          onChange={(e) =>
            updateNonLabour(line.id, { title: e.target.value })
          }
          placeholder="e.g., Harness Hire"
        />
      </td>
      ...
      <td>
        ...
        <input
          ...
          placeholder="e.g., Harnesses for 2 x Spot Ops working 3 shows"
        />
      </td>
